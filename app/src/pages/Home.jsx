import FloatingBubbles from "../components/Bubbles";
import Menu from "../components/Menu";

export default function Home() {
    return <div className="w-screen h-screen text-white">
        <div className="absolute w-full h-full z-10">
            <FloatingBubbles />
        </div>
        <Menu />

        <article className=" w-full mt-10 px-8 sm:px-36 self-introduction relative z-20 text-left text-md sm:text-lg">
            <p className="py-3">
                Hi there 👋🏻, My name is <b className="text-green-600">Lucas Diaz</b> and I'm a Fullstack Developer with a Frontend specialization. In 2021, I began my journey in digital interaction and web programming degree, in UOC (Universitat Oberta de Catalunya), followed by an intensive Fullstack Web Design bootcamp this year (2023). I excel in clean code, best practices, and 3D design, particularly using Three.js & R3F.
            </p>

            <p className="py-3">
                I thrive on challenges, love teamwork, and am dedicated to continuous improvement. I'm currently seeking a role that fosters both personal and professional growth.
            </p>
            <p className="py-3">
                Let's connect to explore how I can contribute to your team's success 👉🏻 <a href="mailto:lucasdamian30@gmail.com" className="text-green-600 animate-pulse">lucasdmaian30@gmail.com</a>
            </p>
        </article>
    </div >
}